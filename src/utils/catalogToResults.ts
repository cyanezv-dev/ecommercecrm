...
export function mapCatalogProductToTire(product: unknown): Tire {
  ...
  const name = String(p.name ?? p.titulo ?? '')
  const brand = String(p.brand ?? p.marca ?? '')
  const unit = Number(p.price_offer ?? p.precioOferta ?? 0)
  const normal = Number(p.price_normal ?? p.precioNormal ?? 0)
  const medida = String(cf.medida ?? p.medida ?? '')
  ...
  const img = String(p.photo_url ?? p.foto ?? '').trim()
